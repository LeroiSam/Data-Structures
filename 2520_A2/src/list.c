...
void removeFromFront(list * head, void (*destroy_contents)(ListElement *))
{
    ...
    if (head == NULL) {
        ...
        destroyNode(temp, destroy_contents);
    }
}
...
void destroyList(list * head, void (*destroy_contents)(ListElement *))
{
    ...
    while (nextRecord != NULL)
    {
        ...
        destroyNode(currentRecord, destroy_contents);
    }
    free(head);
}
...
node * init_node(ListElement * temp){
...
void destroyNode(node * toDelete, void (*destroy_contents)(ListElement *)){
    destroy_contents(toDelete->value);
    free(toDelete);
}
...
*************************************************/
ListElement * getValue(node * temp){
    if (temp == NULL){
